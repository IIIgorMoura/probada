...
# *********************************************************************
# Exemplo de código para a aula 7
# *********************************************************************
...
if __name__ == "__main__":  # se o arquivo estiver sendo executado diretamente
    ...
    resultado = engine.aula_7()             
    print("\nTeste 7 - aula7")
    print(resultado)
    save_json("alt_teste_saida_aula7.json", resultado)
    
    dados_correlacionados1 = []
    dados_correlacionados2 = []
    correlacao = pearson_correlation(dados_correlacionados1, dados_correlacionados2)


# ------

    dados = [
        1184.7, 1098.3, 1265.9, 980.4, 1342.8,
        875.2, 1450.6, 1588.1, 1210.5, 1134.9,
        990.7, 1298.2, 1495.4, 910.1, 1088.6,
        1225.0, 1366.8, 995.2, 1179.4, 1620.3,
        1045.7, 1258.8, 1410.9, 960.5, 1322.6,
        1140.1, 1515.7, 1188.0, 1015.3, 1284.4,
        18450.0, -9250.0
    ]

    min_max_norm = min_max_normalize(dados)
    z_norm = z_score_standardize(dados)

    media_b =safe_mean(dados)
    mediana_b =safe_median(dados)
    var_b =safe_variance(dados)
    dp_b =safe_std(dados)    

    
    media_a_min_max =safe_mean(min_max_norm)
    mediana_a_min_max =safe_median(min_max_norm)
    var_a_min_max =safe_variance(min_max_norm)
    dp_a_min_max =safe_std(min_max_norm)
    
    media_a_z =safe_mean(z_norm)
    mediana_a_z =safe_median(z_norm)
    var_a_z =safe_variance(z_norm)
    dp_a_z =safe_std(z_norm)

    iqr = iqr_outliers(dados)

    print("\nDados Padrão")
    print(f"Média: {media_b:.4f}")
    print(f"Mediana: {mediana_b:.4f}")
    print(f"Variância: {var_b:.4f}")
    print(f"Desvio Padrão: {dp_b:.4f}")

    print("\nTestes de Normalização Min-Max")
    print(f"Média: {media_a_min_max:.4f}")
    print(f"Mediana: {mediana_a_min_max:.4f}")
    print(f"Variância: {var_a_min_max:.4f}")
    print(f"Desvio Padrão: {dp_a_min_max:.4f}")

    print("\nTestes de Normalização Z-Score")
    print(f"Média: {media_a_z:.4f}")
    print(f"Mediana: {mediana_a_z:.4f}")
    print(f"Variância: {var_a_z:.4f}")
    print(f"Desvio Padrão: {dp_a_z:.4f}")

    print("\nTeste de Outliers utilizando IQR")
    print(iqr)