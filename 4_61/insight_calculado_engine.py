...
# -----------------
    ...
    print("\nTeste - correlação entre: (Temperatura) e (Quantidade de mm de Chuva teve)")
    temperaturas = [28,29,30,31,32,33,34,35,33,32,31,30,29,28,27,26,27,28,29,30]
    mm_chuvas = [5,3,0,0,2,1,0,0,4,6,8,10,12,15,18,20,17,12,8,5]
    r=pearson_correlation(temperaturas,mm_chuvas)
    
    media_temp = safe_mean(temperaturas)
    media_chuva = safe_mean(mm_chuvas)

    mediana_temp = safe_median(temperaturas)
    mediana_chuva = safe_median(mm_chuvas)

    amplitude_temp = safe_max(temperaturas) - safe_min(temperaturas)
    amplitude_chuva = safe_max(mm_chuvas) - safe_min(mm_chuvas)

    desvio_padrao_temp = safe_std(temperaturas)
    desvio_padrao_chuva = safe_std(mm_chuvas)

    variancia_temp = safe_variance(temperaturas)
    variancia_chuva = safe_variance(mm_chuvas)
    
    print("Correlação: ", r)
    print("MÉdias: ", media_temp, " | ", media_chuva)
    print("Medianas: ", "Temperatura: ", mediana_temp, " | ", "MMs de Chuva: ", mediana_chuva)
    print("Amplitudes: ", "Temperatura: ", amplitude_temp, " | ", "MMs de Chuva: ", amplitude_chuva)
    print("Desvios Padrão: ", "Temperatura: ", desvio_padrao_temp, " | ", "MMs de Chuva: ", desvio_padrao_chuva)
    print("Variâncias: ", "Temperatura: ", variancia_temp, " | ", "MMs de Chuva: ", variancia_chuva)
