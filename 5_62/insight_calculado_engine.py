...
if __name__ == "__main__":  # se o arquivo estiver sendo executado diretamente
    ...
    print(resultado)                                       # somente queremos acessar os métodos da classe e não precisamos
    ...
    # Dados de temperatura média de 30 dias
    temperaturas_30_dias = [
        3.3,  4.2,  3.3,  2.8,  1.9,
        0.5, -1.2, -2.0, -0.8,  1.1,
        2.4,  3.0,  1.7,  0.2, -1.5,
        -2.8, -3.1, -1.9,  0.0,  1.3,
    ...
    # Contar quantos dias tiveram temperatura abaixo de zero
    dias_abaixo_zero = 0

    for temp in temperaturas_30_dias:
        if temp < 0:
            dias_abaixo_zero += 1

    # Probabilidade empírica
    resultado_empirico = dias_abaixo_zero / len(temperaturas_30_dias)

    print("Probabilidade empírica da temperatura ficar abaixo de zero:")
    print(resultado_empirico)


# ------- PRECIPITAÇÃO -------


    precipitacao_30_dias = [
        0.0, 2.3, 5.1, 0.0, 1.2,
        0.0, 0.0, 8.4, 3.1, 0.0,
        0.0, 0.5, 4.0, 0.0, 0.0,
        7.2, 1.8, 0.0, 0.0, 0.0,
    ...
    # Contar quantos dias não choveu
    dias_sem_chuva = 0

    for chuva in precipitacao_30_dias:
        if chuva == 0:
            dias_sem_chuva += 1

    # Probabilidade empírica de não chover
    probabilidade_sem_chuva = dias_sem_chuva / len(precipitacao_30_dias)

    print("\nProbabilidade empírica de não chover:")
    print(probabilidade_sem_chuva)


    dados_empresa = load_json("alt_exemplo_entrada_insight_calculado.json")
    engine = InsightCalculadoEngine(dados_empresa)
    resultado = engine.aula_5()             
    print("\nTeste 5 - aula5")
    print(resultado)
    save_json("alt_teste_saida_aula5.json", resultado)